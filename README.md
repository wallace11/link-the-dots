...
The tool strives to be as portable as possible, requiring only Python 3.4+ as a dependency, and is intended to live as part of the dotfiles either as a cloned repository or as a submodule. Check out <> for more information about recommended setups.
...
- **Package** - A directory identical to what's called a GNU Stow software package containing the same directory tree of the destination.
...
3. For each container in the matching section:
    1. Make sure the container is declared at the "general" section and overwrites its keys with the current container's (the current container gets priority).
...
In the above example we have one machine called "computer" and one container called "dotfiles". All packages in `/path/to/src` will be symlinked to `/path/to/dest`. Very simple and straightforward.
...
Those are the same as the command-line arguments, just permanent.
...
By default, all packages are deployed. However, using this key it's possible to set a list of packages from this container to deploy.
...
This option will make the container behave like a single package.
If this option is selected, [`packages`](#packages) will have no effect.
...
###### Some notes
- For each package, either "include" or "exclude" rule can be applied, but not both.
...
While Link The Dots is not completely adherent to the [Unix Philosophy](https://en.wikipedia.org/wiki/Unix_philosophy), it simply does one thing, and strives to do it well, and leaving all the [other stuff](#non-features) to other tools to handle. In other words, it tries to solve a simple problem by not creating a bunch of other ones in the process.
...
It was tested on all of my machines, on different Linux distributions and I made sure to iron all the bugs that I could find.
I also made sure to write a test suite that'd simulate all kinds of scenarios, to make sure nothing gets destroyed.
However, being one person with one set of mind has its limits so if you find this project interesting but afraid to try it, I encourage you to try it with the `--dry-run` option first, or even in a "controlled" environment where `source` contains dummy packages and `destionation` is an empty directory.
